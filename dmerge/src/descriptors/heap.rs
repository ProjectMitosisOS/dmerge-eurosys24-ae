...
impl HeapDescriptor {
    #[inline]
    pub fn unmap_public_heap(&self, file: *mut mitosis::bindings::file) {
        ...
        (&self.vma).into_iter().enumerate().for_each(|(i, m)| {
            ...
            crate::log::debug!("unmap first from 0x{:x} with size {}", vma_start, vma_sz);
