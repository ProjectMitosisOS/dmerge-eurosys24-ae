...
def download_matrix(intent_name):
    ...
    s3_client.fget_object(bucket_name, "ChatBotData/" + intent_name, filename)
    return numpy.loadtxt(filename)


def load_bow():
    filename = "/tmp/bos.txt"
    s3_client.fget_object(bucket_name, "ChatBotData/" + 'bos.txt', filename)
    BOW = []
...
def lambda_handler(event):
    all_intents = load_intents("Indent.json")
    ...
    for w in range(num_workers):
        ...
        ths.append(
            Process(target=run_worker, args=(all_intents, intent_name, skew, BOW, Network_Bound, pre_process_duration)))
